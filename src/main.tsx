...
function normalizeTask(task: ApiTask): Task {
  ...
  return { ...task, requireAiContent: task.requireAiContent === 'Yes' ? 'Yes' : 'No', dueDate, files: task.attachments?.length ?? task.files ?? 0 }
}
...
function App() {
  ...
  useEffect(() => {
    ...
    document.querySelectorAll<HTMLInputElement>('.form-page input[name="firstSubmissionDate"]').forEach((input) => {
    ...
    aiContentField.className = 'field full ai-content-field'
    aiContentField.innerHTML = '<span>REQUIRE AI CONTENT</span><input type="hidden" name="requireAiContent" value="No"><span class="toggle-control" role="group" aria-label="Require AI content"><button type="button" class="toggle-option selected" data-value="No" aria-pressed="true">No</button><button type="button" class="toggle-option" data-value="Yes" aria-pressed="false">Yes</button></span>'
    const aiContentInput = aiContentField.querySelector<HTMLInputElement>('input[name="requireAiContent"]')
    const aiContentButtons = Array.from(aiContentField.querySelectorAll<HTMLButtonElement>('.toggle-option'))
    const setAiContent = (value: string) => {
      if (aiContentInput) aiContentInput.value = value
      aiContentButtons.forEach((button) => {
  ...
  async function submitTask(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      void submitToGhlAndEmail({ ...fields, requireAiContent: String(data.get('requireAiContent') || 'No'), type, submittedBy: userName, attachments })
    } catch {
    ...
    <div className="app-shell">
      <aside className={`sidebar ${mobileNav ? 'sidebar-open' : ''}`}>
        ...
        <nav className="nav-links">
          ...
          <button className={activeView === 'mytasks' ? 'active' : ''} onClick={() => { setActiveView('mytasks'); setStatusFilter('All'); setMobileNav(false) }}><ListTodo size={18} /> My tasks <span className="nav-count">{pendingCount}</span></button>
        </nav>
      ...
      <main className="main-content">
        ...
        {activeView === 'overview' || activeView === 'mytasks' ? <Overview tasks={visibleTasks} allTasks={tasks} statusFilter={statusFilter} setStatusFilter={setStatusFilter} toggleTask={toggleTask} onNew={() => setActiveView('submit')} onOpen={(id) => { setSelectedTaskId(id); setActiveView('detail') }} userName={userName} myTasks={activeView === 'mytasks'} /> : activeView === 'library' ? <BrandLibrary onBack={() => setActiveView('overview')} /> : activeView === 'detail' && selectedTask ? <TaskDetail task={selectedTask} toggleTask={toggleTask} onBack={() => setActiveView('overview')} /> : <SubmissionForm type={type} setType={setType} onSubmit={submitTask} onCancel={() => setActiveView('overview')} />}
      </main>
...
function Overview({ tasks, allTasks, statusFilter, setStatusFilter, toggleTask, onNew, onOpen, userName, myTasks }: { tasks: Task[]; allTasks: Task[]; statusFilter: 'All' | TaskStatus; setStatusFilter: (filter: 'All' | TaskStatus) => void; toggleTask: (id: number) => void; onNew: () => void; onOpen: (id: number) => void; userName: string; myTasks: boolean }) {
  const completed = allTasks.filter((task) => task.status === 'Completed').length
  const pending = allTasks.filter((task) => task.status === 'Pending').length
  const underReview = allTasks.filter((task) => task.status === 'Under review').length
  const submittedUsers = new Set(allTasks.map((task) => task.submittedBy?.trim()).filter(Boolean)).size
  return <section className="page"><div className="page-heading"><div><p className="kicker">{todayLabel()}</p>{myTasks ? <h1>My tasks <span>✦</span></h1> : <WelcomeMessage firstName={userName.split(' ')[0]} />}<p className="subheading">{myTasks ? 'Everything currently assigned to you is collected here.' : 'Keep the good work moving. Here’s what’s on the Lumeo team desk.'}</p></div><button className="primary-button" onClick={onNew}><Plus size={18} /> New request</button></div>
    <div className="stat-grid"><div className="stat-card yellow"><div className="stat-icon"><Clock3 size={19} /></div><span>In progress</span><strong>{String(pending).padStart(2, '0')}</strong><small>Pending tasks</small></div><div className="stat-card coral"><div className="stat-icon"><Send size={18} /></div><span>Awaiting review</span><strong>{String(underReview).padStart(2, '0')}</strong><small>Under review</small></div><div className="stat-card mint"><div className="stat-icon"><CheckCircle2 size={19} /></div><span>Completed this month</span><strong>{String(completed).padStart(2, '0')}</strong><small>Completed tasks</small></div><div className="stat-card dark"><div className="stat-icon"><Users size={18} /></div><span>Users submitted</span><strong>{String(submittedUsers).padStart(2, '0')}</strong><small>Unique GHL users</small></div></div>
    ...
    <div className="filter-row"><div className="segmented"><button className={statusFilter === 'All' ? 'selected' : ''} onClick={() => setStatusFilter('All')}>All <span>{tasks.length}</span></button><button className={statusFilter === 'Pending' ? 'selected' : ''} onClick={() => setStatusFilter('Pending')}>Pending <span>{pending + underReview}</span></button><button className={statusFilter === 'Completed' ? 'selected' : ''} onClick={() => setStatusFilter('Completed')}>Completed <span>{completed}</span></button></div><button className="filter-button"><CalendarDays size={15} /> Sort: Due date <ChevronDown size={14} /></button></div>
